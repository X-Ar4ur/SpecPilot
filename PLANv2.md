...
browser = Browser(
    ...
    allowed_domains=['*.4gaboards.com'],        # 限制 Agent 漫游
...
`allowed_domains=['*.4gaboards.com']` 是关键安全/聚焦约束：LLM 偶尔会"漫游"（如点击外链跳出 demo），这条限制保证 Agent 只在 4ga 域名内活动，既保证测试聚焦，也避免污染外部环境。
...
agent = Agent(
    ...
    tools=tools,
    sensitive_data={
        'fga_user': os.getenv('FGA_TEST_USER'),
        'fga_pass': os.getenv('FGA_TEST_PASS'),
    },
)
```

把"登录"作为一个独立的测试场景跑通，本身就是有价值的功能点测试。
...
- `simple` → `max_steps=10`(场景 1-3 步,给约 3-7 步缓冲)
...
- 敏感字段(账号、密码等)**绝不**进入 `test_data`,而是通过 `Agent(sensitive_data=...)` 的占位符机制注入。LLM 上下文中始终是占位符(如 `fga_user`、`fga_pass`),真值仅在 DOM 填入时由 browser-use 替换。
